...
class Atraksi extends CI_Controller {
    ...
    $tableName = "tb_atraksi";
		...
		if ($links == "all") {
			/* pagination */	
			$total_row		= $this->db->query("SELECT * FROM `tb_atraksi` WHERE jenis = 'buatan'")->num_rows();
			$per_page		= 4;
			...
			$data = array(
				'data' => $this->m_admin->getAtraksi('buatan',$akhir,$awal),
				'page'	=> "home/atraksi_all",
				'pengunjung'=> $this->pengunjung(),
			);

			$data['pagi']	= _page($total_row, $per_page, 4, site_url("atraksi/buatan/".$links));
		}else{
			$buatan = $this->m_admin->getContent($tableName, array('id'=>$links));
			$foto = $this->m_admin->getContent('tb_media', array('id'=>substr($buatan[0]->link_foto, (strpos($buatan[0]->link_foto, 'i/'))+2)));
			$other = $this->m_admin->getOther($tableName, $buatan[0]->jenis);
			$data = array(
				'data' => $buatan,
				'foto' => $foto,
				'other' => $other,
			...
			$upd = array('dilihat'=>($buatan[0]->dilihat + 1));
			$this->m_admin->UpdateData($tableName, $upd, array('id'=>$links));
		}
		$this->load->view('home/st_front', $data);
		// echo "<pre>";
