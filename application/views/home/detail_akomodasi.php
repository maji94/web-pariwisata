		...
		<div class="inner">
			<div class="box">
				...
				<div class="content" style="padding-bottom: 0px;">
					...
					<span class="image fit"><img src="<?php echo base_url('assets/images/'.$this->uri->segment(2).'/'.str_replace('.', '_thumb.', unserialize($d->foto_galeri)[0])); ?>" alt=""></span>
					<br>
					<p style="text-align: left;"><span><i class="icofont-eye-alt"></i> Dilihat : <?php echo ($d->dilihat+1); ?> kali</span></p>
					<br>
					<?php echo $d->konten; ?>
					<br>
					<a href="<?php echo base_url('assets/images/'.$d->jenis.'/'.str_replace('.', '_thumb.', $foto_galeri[0])); ?>" data-lightbox="mygallery" class="button special alt"><i class="fa fa-picture-o" aria-hidden="true"></i> Lihat Foto</a>
					<?php for ($m=1; $m < count($foto_galeri); $m++) { ?>
						<a href="<?php echo base_url('assets/images/'.$d->jenis.'/'.str_replace('.', '_thumb.', $foto_galeri[$m])); ?>" data-lightbox="mygallery">
					<?php } ?>
					...
					<br><hr>
					Share this : <?php echo count($foto_galeri); ?>
				</div>
				<?php } ?>
